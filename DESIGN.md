...
| Cost | 100e |
| HP | 150 |
| Function | Offensive against enemy base. Chooses one enemy tank or building to target at random and begins to path towards it with a weighted random walk and A\* waypoints. If no tanks left then chooses a random enemy building. If its target tank is destroyed it then picks another target at random from the valid set. VIRUS remains cloaked while moving but must uncloak to attack a TANK (~10s kill) or infect a building (~10s channel). Infecting a building destroys the virus; a single virus's channel contribution is based on remaining health at the start of the channel: base 15s at full health (150 HP), prorated linearly (e.g. 75 HP = 7.5s). **Multiple VIRUS channeling the same building share a pooled timer — durations stack**, so more viruses complete the infection faster (two full-health viruses finish in ~7.5s). |
...
| Special | Cloaked by default. Revealed when: within PATROL detection AoE (2-3 tiles, home/contested territory), directly overflown by STRIKE (1 tile), within Avatar LoS (3-4 tiles in FPS mode, 1 tile in RTS mode), or when beginning an attack on a TANK or building (auto-uncloak). **Strategic note:** VIRUS has no direct counter to PATROL — it cannot attack aerial units at all. A VIRUS player must use STRIKE to deplete enemy PATROL before VIRUS can reach its targets. This creates a combined-arms requirement: VIRUS without STRIKE support is ineffective against a prepared defender. **No infection cap:** a player may infect any number of a defender's buildings simultaneously (each infection costs a virus). Each effect expires on its own (~60s) and its **magnitude scales with the completing virus's health** at the start of the channel (strength 0–1, all rates scaled by it). Re-infecting an already-infected building from the same attacker **refreshes** it: the remaining effect time extends by the base duration × the new virus's strength and the strength is replaced. The Avatar can cure any friendly building instantly by touching it in FPS mode; the building the Avatar is currently empowering is immune to infection. |
