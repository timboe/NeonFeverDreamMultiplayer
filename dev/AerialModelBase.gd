@tool
extends Node3D
class_name AerialModelBase

# Shared builders for the AERIAL unit model designs. Each design scene
# (Aerial1.tscn .. Aerial10.tscn) has a root node running Aerial.gd (gameplay)
# and a Body child running a script that extends this and implements
# _build_design(). Visual-only, runs on all peers. @tool so the designs are
# visible when opening the scenes in the editor.
#
# Aerial.gd expects the nodes Body/CSG, Body/Gun and Body/Gun/Muzzle to exist,
# so every design must build a core shell (player-coloured via surface
# override) and a forward-facing gun (+Z barrel, muzzle at the tip). See
# _build_core() and _build_gun().
...
var _flap_nodes: Array[Node3D] = []
var _flap_speeds: Array[float] = []
var _flap_amps: Array[float] = []
var _flap_phases: Array[float] = []
var _flap_axes: Array[Vector3] = []
...
func _process(delta: float) -> void:
	...
	for i in _flap_nodes.size():
		_flap_nodes[i].rotation = _flap_axes[i] * (sin(_t * _flap_speeds[i] + _flap_phases[i]) * _flap_amps[i])
...
func _add_flap(node: Node3D, speed: float, amp: float, phase: float = 0.0, axis: Vector3 = Vector3.RIGHT) -> void:
	_flap_nodes.append(node)
	_flap_speeds.append(speed)
	_flap_amps.append(amp)
	_flap_phases.append(phase)
	_flap_axes.append(axis)

# --- Contract builders (Aerial.gd expects Body/CSG, Body/Gun, Body/Gun/Muzzle) ---

func _build_core(mesh: Mesh) -> void:
	_csg.mesh = mesh
	_csg.material_override = _mat(Color(0.55, 0.6, 0.68), 0.9, 0.35)

func _build_gun(pivot: Vector3, barrel_len: float, barrel_radius: float, mat: Material) -> void:
	$Gun.position = pivot
	$Gun/Muzzle.position = Vector3(0, 0, barrel_len)
	var barrel := _part($Gun, _cyl(barrel_radius, barrel_radius, barrel_len, GUN_SEGMENTS), mat, Vector3(0, 0, barrel_len * 0.5), Vector3(90, 0, 0))
	barrel.name = "Barrel"
