...
func apply_toggle(pnum: int, toggle_tile_id: int) -> void:
	if not multiplayer.is_server():
		return
	if not tile_dictionary.has(toggle_tile_id):
		push_warning("TileManager.apply_toggle: unknown tile_id ", toggle_tile_id)
		return
	...
	if tile.state != TileManager.State.RAISED and tile.state != TileManager.State.LOWERED:
		push_warning("TileManager.apply_toggle: tile ", toggle_tile_id, " is not RAISED or LOWERED (state=", tile.state, ")")
		return
	if tile.state == TileManager.State.LOWERED and tile.building != null:
		push_warning("TileManager.apply_toggle: tile ", toggle_tile_id, " is LOWERED but has a building (state=", tile.building, ")")
		return
